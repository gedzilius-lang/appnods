        ...
        <div className="animate-fade-in flex flex-col h-[calc(100vh-260px)]">
            <div className="flex space-x-2 mb-4 overflow-x-auto no-scrollbar">
                {channels.map(channel => (
                    <button key={channel} onClick={() => setActiveChannel(channel)}
                        className={`flex-shrink-0 px-4 py-2 text-sm font-semibold rounded-full transition-colors ${activeChannel === channel ? 'bg-purple-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>
                        #{channel}
                    </button>
                ))}
            </div>
            <div className="flex-grow bg-gray-900 border border-gray-800 rounded-lg p-4 overflow-y-auto no-scrollbar">
                {(messages[activeChannel] || []).map(msg => (
                    <div key={msg.id} className={`flex items-start mb-4 ${msg.author.nickname === mockUser.nickname ? 'flex-row-reverse' : 'flex-row'}`}>
                         ...
                         {msg.author.nickname === mockUser.nickname && <img src={msg.author.avatarUrl} alt={msg.author.nickname} className="w-8 h-8 rounded-full ml-3" />}
                        <div className={`p-3 rounded-lg max-w-xs ${msg.author.nickname === mockUser.nickname ? 'bg-purple-700' : 'bg-gray-800'}`}>
                            <p className="text-white text-sm">{msg.content}</p>
                            <p className={`text-xs text-gray-400 mt-1 ${msg.author.nickname === mockUser.nickname ? 'text-left' : 'text-right'}`}>{msg.author.nickname === mockUser.nickname ? `@ ${msg.timestamp}` : `${msg.author.nickname} @ ${msg.timestamp}`}</p>
                        </div>
                    </div>
                ))}
